...
class Face(FaceParts):
    def __init__(self, bbox: np.ndarray, landmarks: np.ndarray, landmarks_eyes:Optional[np.ndarray]=None, landmarks3D:Optional[np.ndarray]=None):
        super().__init__(FacePartsName.FACE)
        self.bbox = bbox
        self.landmarks = landmarks
        self.landmarks_eyes = landmarks_eyes
...
class FaceModel:
    ...
    def estimate_head_pose(self, face: Face, camera: Camera,
                           rvec_guess = np.zeros(3, dtype=np.double),
                           tvec_guess = np.array([0, 0, 1], dtype=np.double)) -> None:
        ...
        #rvec = np.zeros(3, dtype=np.double)
        #tvec = np.array([0, 0, 1], dtype=np.double)
    ...
    def compute_face_eye_centers(self, face: Face) -> None:
        ...
        face.center = face.model3d[np.concatenate(
                [self.REYE_INDICES, self.LEYE_INDICES,
                 self.NOSE_INDICES])].mean(axis=0)    # The 3D face model is centered in (0,0,0), so this is the corresponding point
        face.centroid = face.model3d.mean(axis=0)
    ...
    the eyes and nose, the X axis is parallel to the line passing
    ...
    LANDMARKS: ClassVar[np.ndarray] = np.array([
        [0.0, 0.02279539, 0.01496097], #0
        [0.0, 0.0, 0.0], # 1 # NOSE TIP
        [0.0, 0.00962159, 0.01417337], # 2
        [-0.00463928, -0.02082222, 0.00842021], #3
        [0.0, -0.00663695, -0.00110976], #4
        [0.0, -0.01492534, 0.00232734], #5
        [0.0, -0.0360012, 0.01686977], #6
        [-0.04253081, -0.03704511, 0.04195902], #7
        [0.0, -0.05145907, 0.0219084], #8
        [0.0, -0.06012844, 0.02090346], # 9
        [0.0, -0.09388643, 0.02994069], #10
        [0.0, 0.02579946, 0.0161068],  #11
        [0.0, 0.02791436, 0.01906174], #12
        [0.0, 0.02867571, 0.02256122], #13
        [0.0, 0.03415535, 0.0207085],  #14
        [0.0, 0.03618712, 0.01946147], #15
        [0.0, 0.03892702, 0.01874156], #16
        [0.0, 0.04238258, 0.01940163], #17
    ...
    REYE_INDICES: ClassVar[np.ndarray] = np.array([33, 133])
    LEYE_INDICES: ClassVar[np.ndarray] = np.array([362, 263])
    MOUTH_INDICES: ClassVar[np.ndarray] = np.array([78, 308])
    NOSE_INDICES: ClassVar[np.ndarray] = np.array([240, 460])

    LANDMARKS_ORIGIN = LANDMARKS[np.concatenate([REYE_INDICES,LEYE_INDICES,NOSE_INDICES])].mean(axis=0)
    #LANDMARKS_ORIGIN = np.mean(LANDMARKS, axis=0)
    LANDMARKS = LANDMARKS - LANDMARKS_ORIGIN # WE CENTER THE 3D FACE MODEL IN (0,0,0)

    CHIN_INDEX: ClassVar[int] = 152 # 199
    NOSE_INDEX: ClassVar[int] = 1

    OUTER_HEAD_POINTS = [162, 389]
    NOSE_BRIDGE = 6
    NOSE_TIP = 4

    LEFT_EYE_DICT = {"top": [27, 28], "bottom": [23, 24], "left": [130], "right": [243], "pupil": [468],
                        "vertical_pair1": [153, 158], "vertical_pair2": [145, 159], "horizontal_pair": [33, 133],
                        "adjacent_eyelid": [160, 159, 158, 163, 144, 145, 153], "iris": [469,470,471,472]}
    RIGHT_EYE_DICT = {"top": [257, 258], "bottom": [253, 254], "left": [463], "right": [359], "pupil": [473],
                         "vertical_pair1": [374, 386], "vertical_pair2": [380, 385], "horizontal_pair": [362, 263],
                         "adjacent_eyelid": [387, 386, 385, 390, 373, 374, 380], "iris": [474,475,476,477]}

    EYEBALL_RADIUS = 0.01620996672680814
    LEFT_EYEBALL_CENTER = np.array([-0.029594026866455746, -0.039910371168937114, 0.052042211586903815]) #np.array([-0.03126769,-0.03967632,0.04402847])
    RIGHT_EYEBALL_CENTER = np.array([0.029594026866455746, -0.039910371168937114, 0.052042211586903815]) #np.array([0.03126769,-0.03967632,0.04402847])
